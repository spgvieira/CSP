...
void partitionInput(int numThread, int start, int end, int numPartitions,
                    std::vector<std::vector<std::tuple<int64_t, int64_t>>>& threadLocalPartitions) {
    for (int i = start; i < end; i++) {
        tuple<int64_t, int64_t> t = input[i];
        int partitionKey = hashFunction(get<0>(t), numPartitions);
        threadLocalPartitions[partitionKey].push_back(t); // Corrected writing!
    }
}
...
int main(int argc, char* argv[]) {
    ...
    std::vector<std::thread> threads(numThreads);
    std::vector<std::vector<std::vector<std::tuple<int64_t, int64_t>>>> threadPartitions(
        numThreads, std::vector<std::vector<std::tuple<int64_t, int64_t>>>(numPartitions));
